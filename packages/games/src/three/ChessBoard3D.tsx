...
function getCameraPreset(mode: BoardCameraMode, orientation: BoardOrientation) {
  ...
  return {
    fov: 40,
    position: [0, 12.4, 8.2 * side] as const,
...
function ChessScene({
  ...
  return (
    <>
      ...
      <directionalLight position={[5, 10, 5]} intensity={1.25} castShadow />
      <pointLight color={CYAN} intensity={6} position={[-3.6, 3.4, 3.6]} distance={8} />
      <pointLight color={MAGENTA} intensity={4} position={[3.8, 3.2, -3.4]} distance={8} />
...
export function ChessBoard3D({
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      >
        <Canvas
          camera={{ fov: 40, position: [0, 12.4, 8.2], near: 0.1, far: 100 }}
          dpr={[1.25, 2]}
          frameloop="demand"
          shadows
